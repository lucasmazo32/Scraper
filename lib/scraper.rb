...
class Scraper
  ...
  def loc_link
    links = @doc.xpath("//div[@id='rb_Location']//li//a//attribute::href")
    links = webpage(@page_url).xpath("//div[@id='rb_Location']//li//a//attribute::href") while links.empty?
    links.to_a
    links[0...3].map { |x| @in_job_url + x.to_s }
  end
  ...
  def years_of_experience(city_number)
    arr = []
  ...
  # It gives the array needed with two parameters, n, 1 for Salary, 2
  # for Job type and 3 for Location and the Link of the page
